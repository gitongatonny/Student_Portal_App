...
public class LecLandingActivity extends AppCompatActivity {
                ...
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        if (!task.getResult().isEmpty()) {
                            // Course is already registered by another lecturer
    ...
    private void performCourseRegistration() {
        String selectedCourse = courseSpinner.getSelectedItem().toString();

        // Update the lec's details in Firestore
        db.collection("lecturer_details")
                .document(mAuth.getUid())
                .update("teaching_course", selectedCourse)
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
